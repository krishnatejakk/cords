# -*- coding: utf-8 -*-
"""Copy of Modular_Training_CRAIG_Strategies_1.ipynb
...
def train_model(num_epochs, dataset_name, datadir, feature, model_name, fraction, select_every, learning_rate, run,
    ...
    print_every = 3
    all_logs_dir = 'results/' + strategy + '/' + dataset_name + '/' + feature + '/' + str(
    ...
    for i in range(num_epochs):
        ...
        if strategy in ['Random-Online']:
            ...
            pass
        elif (strategy in ['GLISTER', 'GradMatch', 'GradMatchPB', 'CRAIG']) and (((i + 1) % select_every) == 0):
            start_time = time.time()
            ...
            idxs = subset_idxs
            if strategy in ['GradMatch', 'GradMatchPB', 'CRAIG']:
            ...
            if i < 99:
                ...
                cached_state_dict = copy.deepcopy(model.state_dict())
                clone_dict = copy.deepcopy(model.state_dict())
                if strategy == 'CRAIG-Explore':
                    subset_idxs, gammas = setf_model.select(int(bud), clone_dict, 'lazy')
                else:
                    subset_idxs, gammas = setf_model.select(int(bud), clone_dict)
                model.load_state_dict(cached_state_dict)
                idxs = subset_idxs
                if strategy in ['GradMatch-Explore', 'GradMatchPB-Explore', 'CRAIG-Explore']:
                    gammas = torch.from_numpy(np.array(gammas)).to(device).to(torch.float32)
            subset_selection_time += (time.time() - start_time)
        ...
        if strategy in ['CRAIG', 'CRAIG-Explore', 'GradMatch', 'GradMatch-Explore', 'GradMatchPB',
                        'GradMatchPB-Explore']:
            start_time = time.time()
