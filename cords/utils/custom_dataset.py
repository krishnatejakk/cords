...
def load_dataset_custom(datadir, dset_name, feature, isnumpy=False):
    if dset_name == "dna":
        ...
        trainset = torchvision.datasets.ImageNet(root="/home/kxk190041/Datasets/data/imagenet/",
                                      split='train',
                                      transform=train_transforms)


        valset = torchvision.datasets.ImageNet(root="/home/kxk190041/Datasets/data/imagenet/",
                                                 split='val',
                                                 transform=val_transforms)

        testset = torchvision.datasets.ImageNet(root="/home/kxk190041/Datasets/data/imagenet/",
                                               split='val',
                                               transform=val_transforms)
        ...
        if feature == 'classimb':
            samples_per_class = torch.zeros(num_cls)
            for i in range(num_cls):
                samples_per_class[i] = len(torch.where(trainset.targets == i)[0])
            min_samples = int(torch.min(samples_per_class) * 0.1)
            selected_classes = np.random.choice(np.arange(num_cls), size=int(0.3 * num_cls), replace=False)
            for i in range(num_cls):
                if i == 0:
                    if i in selected_classes:
                        subset_idxs = list(
                            np.random.choice(torch.where(trainset.targets == i)[0].cpu().numpy(), size=min_samples,
                                             replace=False))
                    else:
                        subset_idxs = list(torch.where(trainset.targets == i)[0].cpu().numpy())
                else:
                    if i in selected_classes:
                        batch_subset_idxs = list(
                            np.random.choice(torch.where(trainset.targets == i)[0].cpu().numpy(), size=min_samples,
                        ...
                        batch_subset_idxs = list(torch.where(trainset.targets == i)[0].cpu().numpy())
                    subset_idxs.extend(batch_subset_idxs)
            trainset = torch.utils.data.Subset(trainset, subset_idxs)
        return trainset, valset, testset, num_cls